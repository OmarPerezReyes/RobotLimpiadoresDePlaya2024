...
def detectar_tonalidad(imagen):
    ...
    total_pixeles1 = np.prod(imagen.shape[:2])
    pixeles_azul_celeste = cv2.countNonZero(mask1)
    porcentaje_azul_celeste = (pixeles_azul_celeste / total_pixeles1) * 100
    
      # Calcular el porcentaje de píxeles negros
    total_pixeles2 = np.prod(imagen.shape[:2])
    pixeles_negros = cv2.countNonZero(mask2)
    porcentaje_negro = (pixeles_negros / total_pixeles2) * 100
...
porcentaje_negro, porcentaje_negro, imagen_salida = detectar_negro(imagen)
